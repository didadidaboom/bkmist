...
class GetAddressMomentModelSerializer(ModelSerializer):
    ...
    def get_moment_list(self,obj):
        address_query = models.MomentCiteAddressRecord.objects.filter(moment__moment_status=0,address__addressGeo=obj).all().order_by("-id")
        ...
        for item in address_query:
            ...
            if (day_ori):
                ...
                if (hour_ori > 1):
                    moment["create_date"] = str(hour_floor) + "小时前"
                else:
                    if (minute_ori > 1):
                        moment["create_date"] = str(minute_floor) + "分钟前"
                    else:
